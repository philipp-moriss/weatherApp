...
export const fetchThunkWeatherTC = (title:string) => (dispatch:Dispatch) => {
    API.searchWeatherByCiti(title)
        .then(({data}) => {
            const {main, weather, name} = data
            const {temp_min, feels_like} = main
            /*setData({...Data,temp_min,feels_like,name,Error:''})*/
        ...
        .catch((rej) => {
            const {message} = rej
            /*setData({...Data, name: '',Error:message})*/
